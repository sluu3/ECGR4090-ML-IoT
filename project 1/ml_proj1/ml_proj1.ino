...
constexpr int kTensorArenaSize = 85000;
...
void setup() {
  ...
  TF_LITE_REPORT_ERROR(error_reporter, "Start setup");
  delay(5000);
  ...
  if (micro_op_resolver.AddDepthwiseConv2D() != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "Added conv2d");
    return;
  }
  if (micro_op_resolver.AddFullyConnected() != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "added fully connected ");
    return;
  }
  if (micro_op_resolver.AddSoftmax() != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "added soft max ");
    return;
  }
  if (micro_op_resolver.AddReshape() != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "add reshape ");
    return;
  }
  ...
  if ((model_input->dims->size != 4) || (model_input->dims->data[0] != 1) ||
      (model_input->dims->data[1] != kFeatureSliceCount) || (model_input->dims->data[2] != kFeatureSliceSize) ||
